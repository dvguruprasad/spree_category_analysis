    ...
    module Admin
        class ProductWeeklyInventoryPosition < ActiveRecord::Base
            ...
            def self.inventory_positions(sales)
                ...
                closing_position = inventory_positions.empty? ? closing_position(sales.first.product_id) : inventory_positions.last.closing_position
            ...
            def self.create_inventory_position(sale, closing_position)
                ...
                ProductWeeklyInventoryPosition.new(:product_id => sale.product_id,
                                                   :closing_position => computed_inventory_position, :week_start_date => sale.week_start_date)
            end

            def self.replenishment_if_any(sale)
                replenishment = InventoryReplenishment.find_by_product_id_and_replenishment_date(sale.product_id, sale.week_start_date)
