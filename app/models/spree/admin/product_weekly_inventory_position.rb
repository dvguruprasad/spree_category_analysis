module Spree
    module Admin
        class ProductWeeklyInventoryPosition < ActiveRecord::Base
            ...
            attr_accessible :child_id, :closing_position, :week_start_date
            ...
            def self.create_inventory_position(sale, closing_position)
                ...
                ProductWeeklyInventoryPosition.new(:child_id => sale.child_id,
                                                   :closing_position => computed_inventory_position, :week_start_date => sale.week_start_date)
            end

            def self.replenishment_if_any(sale)
                replenishment = InventoryReplenishment.find_by_child_id_and_replenishment_date(sale.child_id, sale.week_start_date)
                replenishment.nil? ? 0 : replenishment.quantity
            end

            def self.closing_position(child_id)
                Spree::Admin::ProductWeeklyInventoryPosition.last(:conditions => ["child_id = ?", child_id], :order => "week_start_date").closing_position
            end
        end
    end
end
